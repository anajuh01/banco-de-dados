...
SELECT  contagem ( produto_id ) FROM produtos;
SELECIONE  contagem ( nome ) FROM produtos;

# 2:

@@ -17,18 +19,21 @@ ONDE preco ENTRE 50 e 150;

# 5:
SELECIONE nome, preço FROM produtos
PEDIDO POR preco desc
PEDIDO POR preço asc
LIMITE  5 ;

# 6:
SELECIONE o nome DE produtos
ONDE qtd_estoque =  0 ;

# 7:
SELECIONE nome, descrição FROM produtos
WHERE descrição !=  null  ou descrição !=  ' ' ;
WHERE descricao não é nula  ou descricao !=  ' ' ;

# 8:

SELECIONE nome, preço FROM produtos
ONDE preço >  AVG (preco);
WHERE preço >  ( SELECT AVG (preco ) FROM produtos ); 

# 9:
SELECIONE  *  DOS produtos
 -39,7 +44,9 SELECT nome, preço FROM produtos
PEDIDO POR preco desc ;

# 11:
SELECT nome, preço
ONDE  preco % 2  =  0 ;
SELECIONE nome, qtd_estoque FROM produtos
ONDE  qtd_estoque % 2  =  0 ;

# 12:
SELECIONE nome, descrição FROM produtos
ONDE preço <  100 . 00  AND qtd_estoque >  0 ;
